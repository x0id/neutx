 ...
 *
 * This is read-only complement to neutx::container::detail::svector class.
 *
...
#ifndef _NEUTX_CONTAINER_DETAIL_SARRAY_HPP_
#define _NEUTX_CONTAINER_DETAIL_SARRAY_HPP_
...
public:
    ...
    std::pair<bool, const Data*> get_left(symbol_t a_symbol) const {
        ...
        m_map.index(m_mask, a_symbol, l_mask, l_index);
        if ((l_mask & m_mask) != 0)
            return std::make_pair(false, &m_array[l_index]);
        else if (l_index > 0)
            return std::make_pair(true, &m_array[l_index - 1]);
        else
...
#endif // _NEUTX_CONTAINER_DETAIL_SARRAY_HPP_
