...
#include <neutx/container/detail/pnode_ro.hpp>
#include <neutx/container/ptrie.hpp>
#include <neutx/container/mmap_ptrie.hpp>
#include <neutx/container/detail/simple_node_store.hpp>
#include <neutx/container/detail/flat_data_store.hpp>
...
#include <neutx/memstat_alloc.hpp>
...
struct f0 {
    ...
    static bool lookup(const data_t*& ret, const data_t& data, const store_t&,
            uint32_t, bool) {
        if (!data.empty())
            ret = &data;
        return true;
...
struct f2 {
    ...
    static bool copy_exact_f(std::string &acc, offset_t off,
            const store_t& store, uint32_t, bool has_next) {
    ...
    static bool lookup_simple(const data*& ret, offset_t off,
            const store_t& store, uint32_t, bool) {
        if (off == store_t::null)
            return true;
        data *ptr = store.native_pointer<data>(off);
        if (ptr == NULL)
...
BOOST_FIXTURE_TEST_CASE( compact_test, f1 )
{
    ...
    encoder_t::store_type store("test-trie.bin");
    encoder_t encoder;
